...
from src.open_deep_research.state import ReportStateInput, ReportStateOutput, Sections, ReportState, SectionState, \
    SectionOutputState, Queries, Feedback
from src.open_deep_research.prompts import report_planner_query_writer_instructions, report_planner_instructions, \
...
from src.open_deep_research.utils import tavily_search_async, deduplicate_and_format_sources, format_sections, \
    perplexity_search


# Get writer model
def get_writer_model(config: RunnableConfig):
    ...
    if writer_provider == "openai":
        writer_model = ChatOpenAI(model=configurable.writer_model, temperature=0)
    elif writer_provider == "anthropic":
        writer_model = ChatAnthropic(model=configurable.writer_model, temperature=0)
    else:
        raise ValueError(f"Unsupported writer provider: {Configuration.writer_provider}")
    return writer_model
...
async def generate_report_plan(state: ReportState, config: RunnableConfig):
    ...
    # Get writer model
    writer_model = get_writer_model(config)
...
def human_feedback(state: ReportState, config: RunnableConfig) -> Command[
    ...
    if isinstance(feedback, bool) or feedback.lower().strip() == "true":
...
def write_section(state: SectionState, config: RunnableConfig) -> Command[Literal[END, "search_web"]]:
    ...
    # Grade prompt
    section_grader_instructions_formatted = section_grader_instructions.format(section_topic=section.description,
